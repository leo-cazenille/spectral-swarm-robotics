...
// XXX
void draw_bot_comms(SDL_Surface *surface, int w, int h, kilobot *bot)
{
  ...
  if (simparams->showCommsRadius) {
    if (bot->tx_enabled == 1) {
      Uint32 comm_color = LEDcolor(bot, 80);
      filledCircleColor(surface, draw_x, draw_y, scale * bot->cr, comm_color);
      if (colorscheme->anti_alias)
          aacircleColor(surface, draw_x, draw_y, scale * bot->cr, comm_color);
      else
          circleColor(surface, draw_x, draw_y, scale * bot->cr, comm_color);      
    }
  }
