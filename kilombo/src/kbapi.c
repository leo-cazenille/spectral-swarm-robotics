 ...
 * We store the distance in mm in the high_gain field when passing messages,
 * so we can simply return it here.
 * Version only for the simulator, returning a uint16_t instead of the uint8_t of the kilolib.
 */
uint16_t estimate_distance_simulator(const distance_measurement_t *d)
{
  return d->high_gain;
}
