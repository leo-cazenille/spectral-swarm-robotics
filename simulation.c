...
char *botinfo(void) {
    ...
    p += sprintf (p, "ID: %d   X: %f   Y: %f   s_1: %f   nb_neighbours: %d   nb_known_neighbors: %d \n", kilo_uid, current_bot->x, current_bot->y, mydata->diff1.s[0], mydata->nb_neighbors, mydata->nb_known_neighbors);
#else
    p += sprintf (p, "ID: %d   X: %f   Y: %f   s_1: %f   nb_neighbours: %d \n", kilo_uid, current_bot->x, current_bot->y, mydata->diff1.s[0], mydata->nb_neighbors);
#endif
    p += sprintf (p, "lambda: %f   avg_lambda: %f   coll_avg_step: %d\n", mydata->diff1.lambda, mydata->diff1.avg_lambda, mydata->current_coll_avg_step);
